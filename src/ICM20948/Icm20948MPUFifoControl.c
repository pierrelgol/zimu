...
int dmp_reset_fifo(struct inv_icm20948 *s) {
        printf("\n###########\nReseting fifo, IMU thread overrun.\n###########\n");
...
static uint_fast16_t get_packet_size_and_samplecnt(unsigned char *data, unsigned short *header, unsigned short *header2, unsigned short *sample_cnt_array) {
        ...
        *header = (((unsigned short)data[0]) << 8) | data[1];
        ...
        if (*header & ACCEL_SET) {
                sz += ACCEL_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_ACCELEROMETER]++;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_RAW_ACCELEROMETER]++;
        }

        if (*header & GYRO_SET) {
                sz += GYRO_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_GYROSCOPE_UNCALIBRATED]++;
                sz += GYRO_BIAS_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_GYROSCOPE]++;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_RAW_GYROSCOPE]++;
        }

        if (*header & CPASS_SET) {
                sz += CPASS_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_MAGNETIC_FIELD_UNCALIBRATED]++;
        }

        if (*header & ALS_SET) {
                sz += ALS_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_LIGHT]++;
        }

        if (*header & QUAT6_SET) {
                sz += QUAT6_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_GAME_ROTATION_VECTOR]++;
        }

        if (*header & QUAT9_SET) {
                sz += QUAT9_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_ROTATION_VECTOR]++;
        }

        if (*header & PQUAT6_SET) sz += PQUAT6_DATA_SZ;

        if (*header & GEOMAG_SET) {
                sz += GEOMAG_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_GEOMAGNETIC_ROTATION_VECTOR]++;
        }

        if (*header & CPASS_CALIBR_SET) {
                sz += CPASS_CALIBR_DATA_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_GEOMAGNETIC_FIELD]++;
        }

        if (*header & PED_STEPDET_SET) {
                sz += PED_STEPDET_TIMESTAMP_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_STEP_DETECTOR]++;
        }
        ...
        if (*header2 & FLIP_PICKUP_SET) {
                sz += FLIP_PICKUP_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_FLIP_PICKUP]++;
        }
        if (*header2 & ACT_RECOG_SET) {
                sz += ACT_RECOG_SZ;
                if (sample_cnt_array) sample_cnt_array[ANDROID_SENSOR_ACTIVITY_CLASSIFICATON]++;
        }
        sz += ODR_CNT_GYRO_SZ;
...
static int check_fifo_decoded_headers(unsigned short header, unsigned short header2) {
        ...
        if (header & ~header_bit_mask) return -1;
        ...
        if (header & HEADER2_SET) {
                ...
                if (header2 == 0) return -1;
                if (header2 & ~header2_bit_mask) return -1;
        }
...
static int extract_sample_cnt(struct inv_icm20948 *s, int fifo_size, unsigned short *total_sample_cnt, unsigned short *sample_cnt_array) {
        ...
        while (fifo_idx < fifo_size) {
                ...
                if (fifo_size - fifo_idx < need_sz) goto endSuccess;
...
int inv_icm20948_fifo_swmirror(struct inv_icm20948 *s, int *fifo_sw_size, unsigned short *total_sample_cnt, unsigned short *sample_cnt_array) {
        ...
        *total_sample_cnt = 0;
        ...
        if (extract_sample_cnt(s, *fifo_sw_size, total_sample_cnt, sample_cnt_array)) goto error;
...
int inv_icm20948_fifo_pop(struct inv_icm20948 *s, unsigned short *user_header, unsigned short *user_header2, int *fifo_sw_size) {
        ...
        if (*fifo_sw_size > 3) {
                ...
                fifo_ptr += HEADER_SZ;
                if (fd.header & HEADER2_SET) fifo_ptr += HEADER2_SZ;
                ...
                *fifo_sw_size -= need_sz;
                if (*fifo_sw_size) memmove(fifo_data, &fifo_data[need_sz], *fifo_sw_size); // Data left in FIFO
...
int inv_icm20948_dmp_process_fifo(struct inv_icm20948 *s, int *left_in_fifo, unsigned short *user_header, unsigned short *user_header2, long *time_stamp) {
        ...
        if (*left_in_fifo > 3) {
                ...
                if (user_header) *user_header = fd.header;

                if (user_header2) *user_header2 = fd.header2;
                ...
                *left_in_fifo -= need_sz;
                if (*left_in_fifo) memmove(fifo_data, &fifo_data[need_sz], *left_in_fifo); // Data left in FIFO
        }
